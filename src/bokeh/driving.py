...
def cosine(w: float, A: float = 1, phi: float = 0, offset: float = 0) -> partial[Callable[[], None]]:
    ...
    """
    # Move the import to module scope for efficiency.
    return partial(force, sequence=_advance_cosine(w, A, phi, offset))
...
# Code
#-----------------------------------------------------------------------------
def _advance_cosine(w: float, A: float, phi: float, offset: float) -> Iterator[float]:
    ''' Specialized _advance for cosine series with parameters pre-bound.
    This avoids repeated closure creation and unnecessary python call overhead.
    '''
    from math import \
        cos  # still local to avoid introducing global dependency as per module convention
    i = 0
    arg_add = phi
    # Cache all immutable arguments as local variables for max performance inside the loop.
    while True:
        yield A * cos(w * i + arg_add) + offset
        i += 1
